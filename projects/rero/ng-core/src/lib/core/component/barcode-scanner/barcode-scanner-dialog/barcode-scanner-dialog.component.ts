...
@Component({
  ...
  template: `
  @if (error()) {
    <p-message severity="error" styleClass="core:w-full core:mt-1 core:mb-4">{{ error() }}</p-message>
  }
...
export class BarcodeScannerDialogComponent {
  ...
  private errorMessage(error: unknown): string {
    ...
    switch (name) {
      ...
      case 'SecurityError':
        return this.translateService.instant('Camera access denied');
      case 'NotFoundError':
      case 'OverconstrainedError':
        return this.translateService.instant('No camera found');
      case 'NotReadableError':
        return this.translateService.instant('Camera is already in use');
      default:
        return this.translateService.instant('Unable to start the barcode scanner');
    }
  }
