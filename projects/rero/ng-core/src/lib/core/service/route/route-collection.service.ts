...
export class RouteCollectionService {
   ...
   */
  private readonly collection = signal<Map<string, RouteInterface>>(new Map());
   ...
   */
  routes = computed(() => Array.from(this.collection().values()).map((route) => route.getConfiguration()));
}
