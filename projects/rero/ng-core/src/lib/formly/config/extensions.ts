...
export class NgCoreFormlyExtension {
  ...
  onPopulate(field: FormlyFieldConfig): void {
    ...
    if (expressions && expressions['props.required'] !== undefined && field?.options?.fieldChanges) {
  ...
  private _setWrappers(field: FormlyFieldConfig): void {
    ...
    if (field?.props?.editorConfig?.longMode) {
      ...
      const { parent } = field;
      const isRoot = parent?.props?.isRoot === true;
      const wrappers = field?.wrappers;
      if (isRoot && Array.isArray(wrappers) && !wrappers.includes('card') && !wrappers.includes('hide')) {
        wrappers.unshift('card');
      }
    }
   ...
   */
  private _getKey(field: FormlyFieldConfig): string {
    let parentKey = null;
    const keyParams = [];
   ...
   */
  private _hasHiddenParent(field: FormlyFieldConfig | undefined): boolean {
    if (field?.hide === true) {
   ...
   */
  private _modelIsEmpty(field: FormlyFieldConfig): boolean {
    let model = field?.model;
    // for simple object the model is the parent dict
    if (field.model && !Object.hasOwn(field, 'fieldGroup') && field.key) {
  ...
  private _setCustomValidators(field: FormlyFieldConfig): void {
    if (!field.props || field.props.validation == null) {
      return;
    }
    const customValidators = field.props.validation.validators || {};
    // asyncValidators: valueAlreadyExists
    if (customValidators.valueAlreadyExists && field?.props?.editorConfig?.recordType) {
      const { filter, limitToValues, term } = customValidators.valueAlreadyExists;

      const { recordType, pid } = field.props.editorConfig;

      field.asyncValidators = {
        validation: [
          () =>
            this.recordService.uniqueValue(field, recordType, pid, term ?? null, limitToValues ?? [], filter ?? null),
        ],
      };
    ...
    if (customValidators.uniqueValueKeysInObject) {
      const validators: { keys: string[] } = customValidators.uniqueValueKeysInObject;
      field.validators = {
        uniqueValueKeysInObject: {
          expression: (control: UntypedFormControl): boolean => {
            ...
            const keysToKeep = validators.keys;
            const uniqueItems = Array.from(
              new Set(
                control.value.map((v) => {
                  const keys = keysToKeep.reduce<Record<string, unknown>>((acc, elt) => {
                    acc[elt] = v[elt];
                    return acc;
                  }, {});
    ...
    if (customValidators.numberOfSpecificValuesInObject) {
      const validators: { min: number; max: number; keys: Record<string, string> } =
        customValidators.numberOfSpecificValuesInObject;
      field.validators = {
        numberOfSpecificValuesInObject: {
          expression: (control: UntypedFormControl): boolean => {
            function objIntersection(a: Record<string, unknown>, b: Record<string, unknown>): string[] {
              const keys = Object.keys(a);
              return keys.filter((k) => a[k] === b[k]);
            }
            const min = validators.min || 0;
            const max = validators.max || Infinity;
            const keys = validators.keys;

            // if value isn't an array and no minimum value is specified, no need to check.
            if (!(control.value instanceof Array) && min === 0) {
              return true;
            }
            const counter = control.value.filter(
              (element: Record<string, unknown>) => objIntersection(keys, element).length > 0,
            ).length;
    ...
    if (customValidators.datesGreaterThan) {
      const validators: { dateFirst: string; dateLast: string; strict?: boolean } = customValidators.datesGreaterThan;
      const firstDate = validators.dateFirst;
      const lastDate = validators.dateLast;
      const strict = validators.strict || false;
      field.validators = {
        datesGreaterThan: {
          expression: (control: UntypedFormControl): boolean => {
...
export class TranslateExtension implements FormlyExtension {
  ...
  prePopulate(field: FormlyFieldConfig<FormlyFieldProps & Record<string, unknown>>): void {
  ...
  private processAllAddon(props: Record<string, any>): void {
    if (props.addonLeft) {
  ...
  private processAddon(addon: string[]): string[] {
...
export class FormOptionsProcessExtension implements FormlyExtension {
  ...
  private processOptions(props: Record<string, any>): any[] {
 ...
 */
export function registerNgCoreFormlyExtension(translate: TranslateService): Record<string, unknown> {
  return {
    ...
    validationMessages: [
      ...
      },
      {
        ...
        message: (_err: unknown, field: FormlyFieldConfig) => {
          const validatorConfig = field.props?.validation.validators?.numberOfSpecificValuesInObject;
