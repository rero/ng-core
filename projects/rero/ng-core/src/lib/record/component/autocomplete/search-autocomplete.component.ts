...
export class SearchAutocompleteComponent implements AfterViewInit{
  ...
  suggestions = toSignal<any[], any[]>(this.query.pipe(
  ...
  private getSuggestions(query: string): (Observable<AutoCompleteData[][] | {label: string, items: AutoCompleteData[]}[]>) {
      ...
      map((data: AutoCompleteData[][]) => {
        if (data.length > 1) {
          ...
          return data.map((hits) => {
            if (hits.length > 0) {
              const selectedRecordType = this.recordTypes().find((type: AutoCompleteRecordType) => type.index === hits[0].index);
