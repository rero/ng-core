...
@Component({
    ...
    imports: [BucketsComponent, Fieldset, AggregationSliderComponent, AggregationDateRangeComponent, TranslateDirective, TranslatePipe, UpperCaseFirstPipe]
})
export class RecordSearchAggregationComponent {
  ...
  readonly isAggregationDisplayed = computed(() =>
    this.aggregation().expanded || this.aggregationFilters().length > 0
  );
