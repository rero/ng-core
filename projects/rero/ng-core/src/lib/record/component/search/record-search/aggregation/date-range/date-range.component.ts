...
export class AggregationDateRangeComponent {
  ...
  readonly hasQueryParam = computed(() => this.store.aggregationsFilters().some((f) => f.key === this.key()));
  ...
  updateFilter() {
    ...
    if (model && model[0] && model[1]) {
      ...
      this.store.updateAggregationsFilter(this.key(), [`${model[0].getTime()}--${maxDate}`]);
    }
  }
