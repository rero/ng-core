...
export class AggregationSliderComponent {
  ...
  readonly hasQueryParam = computed(() =>
    this.store.aggregationsFilters().some((f: any) => f.key === this.key())
  );
  ...
  readonly range = linkedSignal<number[]>(() => {
    ...
    return filter
      ? filter.values[0].split('--').map((item: string) => +item)
      : [this.min(), this.max()];
  });
  ...
  updateFilter() {
    const r = [...this.range()];
    if (!r[0] || r[0] < this.min()) { r[0] = this.min(); }
    if (!r[1] || r[1] > this.max()) { r[1] = this.max(); }
