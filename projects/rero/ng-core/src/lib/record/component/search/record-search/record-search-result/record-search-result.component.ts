...
export class RecordSearchResultComponent {
  ...
  private readonly record$ = toObservable(this.record).pipe(
    distinctUntilChanged((a, b) => a?.id === b?.id)
  );

  // Convert observables to signals directly - more efficient than manual subscriptions
  readonly deleteStatus = toSignal(
    this.record$.pipe(switchMap(record => this.store.canDeleteRecord$(record))),
    { initialValue: { can: false, message: '' } }
  );

  readonly updateStatus = toSignal(
    this.record$.pipe(switchMap(record => this.store.canUpdateRecord$(record))),
    { initialValue: { can: false, message: '' } }
  );

  readonly useStatus = toSignal(
    this.record$.pipe(switchMap(record => this.store.canUseRecord$(record))),
    { initialValue: { can: false, message: '' } }
  );

  readonly detailUrl = toSignal(
    this.record$.pipe(switchMap(record => this.resolveDetailUrl(record))),
  ...
  readonly useStatusUrl = computed(() => {
    const status = this.useStatus() as any;
    return status.url;
  });

  readonly updateStatusRouterLink = computed(() => {
    const status = this.updateStatus() as any;
    return status.routerLink;
  });

  readonly deleteStatusType = computed(() => {
    const status = this.deleteStatus() as any;
    return status.type;
  });
