...
export class RecordSearchComponent {
  ...
  readonly addStatus: Signal<ActionStatus> = toSignal(
      this.store.canAddRecord$(),
  ...
  searchInField(event: ToggleSwitchChangeEvent, path: string): void {
    this.store.updateSearchFields(this.store.config().searchFields.map((item: SearchField) => {
      if (item.path === path) {
