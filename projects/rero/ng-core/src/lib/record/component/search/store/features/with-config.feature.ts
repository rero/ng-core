...
export const DEFAULT_RECORD_TYPE: RecordType = {
  ...
  processAggregations: (aggregations) => of(aggregations),
  searchFields: [],
  resultsText: null,
  pagination: {
    boundaryLinks: true,
      ...
      canUpdateRecord$(record: RecordData): Observable<ActionStatus> {
        ...
        if (config.permissions) {
          return config
            .permissions(record)
            .pipe(
              switchMap(perms => ('canUpdate' in perms ? of(perms.canUpdate) : of({ can: true, message: '' }))),
            );
        }
      ...
      canDeleteRecord$(record: RecordData): Observable<ActionStatus> {
        ...
        if (config.permissions) {
          return config
            .permissions(record)
            .pipe(
              switchMap(perms => ('canDelete' in perms ? of(perms.canDelete) : of({ can: true, message: '' }))),
            );
        }
        return of({ can: true, message: '' });
      },
