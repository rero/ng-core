...
export function withSearchFields() {
  return signalStoreFeature(
    ...
    withComputed((state) => ({
      ...
      selectedSearchFields: computed(() => state.searchFields().filter((field) => field.selected === true)),
      /** Check if any search field is selected */
      hasSelectedFields: computed(() => state.searchFields().some((field) => field.selected === true)),
    })),
    withMethods((store) => ({
      /**
      ...
      toggleSearchField(path: string): void {
        const updatedFields = store
          .searchFields()
          .map((field) => (field.path === path ? { ...field, selected: !field.selected } : field));
