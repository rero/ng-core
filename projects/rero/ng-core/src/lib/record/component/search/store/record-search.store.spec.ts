...
describe('RecordSearchStore', () => {
  ...
  describe('facetsParameter', () => {
    ...
    it('should not include aggregations based on expanded state alone', () => {
      store.updateAggregations([{ key: 'author', expanded: true } as any, { key: 'year', expanded: false } as any]);

      // expanded is UI state only — it does not drive facet inclusion
      expect(store.facetsParameter()).toEqual([]);
    });
    ...
    it('should combine all conditions', () => {
      ...
      // year is expanded but not included — expanded does not drive facet inclusion
      expect(store.facetsParameter()).toEqual(['author', 'language']);
    });
