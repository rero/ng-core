...
import { Component, inject, input, model, OnChanges, OnDestroy, OnInit, output, signal, SimpleChanges } from '@angular/core';
...
@Component({
    ...
    imports: [FormsModule, ReactiveFormsModule, Tooltip, Button, SplitButton, Divider, AddFieldEditorComponent, FormlyModule, TranslateDirective, ErrorComponent, AsyncPipe, TranslatePipe, UpperCaseFirstPipe]
})
export class EditorComponent <TMetadata extends JsonObject = JsonObject> extends AbstractCanDeactivateComponent implements OnInit, OnChanges, OnDestroy {

  protected formlyJsonschema: FormlyJsonschema = inject(FormlyJsonschema);
  protected recordService: RecordService = inject(RecordService);
  protected apiService: ApiService = inject(ApiService);
  protected route: ActivatedRoute = inject(ActivatedRoute);
  protected recordUiService: RecordUiService<TMetadata> = inject(RecordUiService);
  ...
  // JSONSchema
  readonly schema = input<any>(undefined);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.schema) { // Schema has changed
    ...
    combineLatest([this.route.params, this.route.queryParams]).subscribe(
      ([params, queryParams]) => {
        ...
        if (this._resourceConfig.editorSettings) {
          this.editorSettings.set({ ...cloneDeep(this.editorSettings()), ...this._resourceConfig.editorSettings });
        }
        ...
        if (queryParams.source === 'templates' && queryParams.pid != null) {
          ...
          record$ = this.recordService
            .getRecord(this.recordType, pid, { headers: editorSettings.getHeaders || new HttpHeaders({ 'Content-Type': 'application/json' }) })
            .pipe(
  ...
  saveAsTemplate(): void {
    ...
    this.ref?.onClose.subscribe(data => {
      if (data) {
        ...
        const recordUIService = this.recordUiService as RecordUiService<any>;
        const tmplConfig = recordUIService.getResourceConfig(this.editorSettings().template.recordType);
        ...
        this.recordService.create<TemplateMetadata>(this.editorSettings().template.recordType, record).subscribe({
          next: (createdRecord) => {
            ...
            recordUIService.redirectAfterSave(
              createdRecord.id,
              createdRecord,
  ...
  removeHiddenField(field: FormlyFieldConfig): void {
    if (this._hiddenFields.some(f => f.id === field.id) && this.isRoot(field.parent ?? null)) {
