...
export class GetRecordPipe implements PipeTransform {
   ...
   */
  transform(pid: string, type: string, returnType = 'object', field?: string, headers?: HttpHeaders | Record<string, string | string[]>): Observable<RecordData | JsonValue | null> {
    ...
    return this.recordService.getRecord(type, pid, { headers: headers || new HttpHeaders({ 'Content-Type': 'application/json' }) }).pipe(
      map(data => {
        if (!data) {
          return null;
        }
